...
const checklist = (items) => Array.isArray(items) && items.length ? `<ul class="tool-checklist">${items.map((c) => `<li class="${c.pass ? 'check-pass' : 'check-fail'}">${icon(c.pass ? 'circle-check' : 'circle-x','w-4 h-4')}<span>${esc(c.label)}</span><b>${c.pass ? 'Pass' : 'Fail'}</b></li>`).join('')}</ul>` : '';
const checkSummary = (items) => Array.isArray(items) && items.length ? `<p class="tool-check-summary">${items.filter((c) => c.pass).length} of ${items.length} checks passed</p>` : '';
...
export function analyzeDecodedQR(decoded) {
  ...
  return { contentType, value, url:null, checks:[ { label: 'Content type identified: ' + contentType, pass: true }, { label: 'URL safety analysis applicable', pass: false }, { label: 'Destination address visible in the code', pass: false } ] };
}
...
function resultPanel(result, id) {
  if (!result) return '';
  if (id === 'tool-01') return `<div class="tool-result-panel strength-result"><h3>Strength analysis</h3><div class="tool-result-heading"><span class="risk-badge ${result.strength.toLowerCase().replace(' ','-')}">${esc(result.strength)}</span><strong>${result.entropyBits} bits estimated entropy</strong></div><p>${esc(result.timeToCrack)}</p><h4>Checks</h4>${checklist(result.checks)}${checkSummary(result.checks)}<h4>Weaknesses</h4>${list(result.weaknesses)}<h4>Improve it</h4>${list(result.recommendations)}<p class="tool-privacy">${esc(result.educationalExplanation)} Your password is evaluated locally and is never transmitted or stored.</p></div>`;
  if (id === 'tool-02') return `<div class="tool-result-panel"><div class="tool-result-heading"><span class="risk-badge ${result.riskLevel.toLowerCase().replace(' ','-')}">${esc(result.riskLevel)}</span><strong>${result.indicators.length} red flag${result.indicators.length === 1 ? '' : 's'}</strong></div><h4>Checks</h4>${checklist(result.checks)}${result.checks?.length ? `<p class="tool-check-summary">${result.indicators.length} of ${result.checks.length} warning signs found</p>` : ''}<h4>Warning signs found</h4>${list(result.indicators)}<h4>Why this matters</h4><p>${esc(result.explanation)}</p><h4>Recommended action</h4><p>${esc(result.recommendedAction)}</p>${result.links?.length ? `<h4>Link hover reveal</h4><ul class="tool-links">${result.links.map((l) => `<li><code>${esc(l)}</code></li>`).join('')}</ul>` : ''}</div>`;
  if (id === 'tool-03') return `<div class="tool-result-panel"><div class="tool-result-heading"><span class="risk-badge ${result.riskLevel.toLowerCase().replace(' ','-')}">${esc(result.riskLevel)}</span><strong>${esc(result.parts.hostname || 'Unrecognized host')}</strong></div><div class="domain-breaker"><span>Subdomain <b>${esc(result.parts.subdomain || '—')}</b></span><span>Domain <b>${esc(result.parts.domain || '—')}</b></span><span>TLD <b>${esc(result.parts.tld || '—')}</b></span></div><h4>Checks</h4>${checklist(result.checks)}${result.checks?.length ? `<p class="tool-check-summary">${result.flags.length} of ${result.checks.length} warning signs found</p>` : ''}<h4>Flags</h4>${list(result.flags)}<h4>Assessment</h4><p>${esc(result.educationalAssessment)}</p><h4>Recommended action</h4><p>${esc(result.recommendedAction)}</p></div>`;
  if (id === 'tool-05') return `<div class="tool-result-panel"><div class="tool-result-heading"><span class="risk-badge low">Report summary ready</span></div><p>Your report stays in this page session and is never transmitted or stored by this tool. Review it, then use your organization\u2019s approved reporting channel yourself.</p>${result.checklist?.length ? `<h4>Recommended next steps (Module 7)</h4><ul class="tool-checklist tool-checklist-steps">${result.checklist.map((x) => `<li>${icon('square','w-4 h-4')}<span>${esc(x)}</span></li>`).join('')}</ul>` : ''}<pre class="report-preview" data-report-text>${esc(result.report)}</pre><div class="report-actions"><button class="secondary-btn" data-copy-report>Copy report</button><button class="secondary-btn" data-download-report>Download .txt</button></div></div>`;
  if (id === 'tool-07') {
    ...
    const ruleRows = `<ul class="log-rule-summary">${Object.keys(result.ruleLabels).map((k) => {
      const hits = result.ruleCounts[k] || 0;
      const risk = hits ? (result.entries.some((e) => e.flags.some((f) => f.rule === k && f.risk === 'critical')) ? 'critical' : result.entries.find((e) => e.flags.some((f) => f.rule === k))?.flags.find((f) => f.rule === k)?.risk || 'medium') : 'none';
      return `<li class="${hits ? '' : 'log-rule-clear'}"><span class="log-rule-pill log-risk-${risk}">${esc(result.ruleLabels[k])}</span><strong>${hits} hit${hits === 1 ? '' : 's'}</strong></li>`;
    }).join('')}</ul>`;
    return `<div class="tool-result-panel"><div class="tool-result-heading"><span class="log-risk-badge log-risk-${esc(result.risk)}">${esc(result.risk)} risk</span><strong>${result.flaggedCount} of ${result.totalLines} lines flagged</strong></div><p>${esc(result.headline)}</p>${result.flaggedCount === 0 ? '<p class="tool-muted">All 7 detection rules ran and none matched.</p>' : ''}<h4>Detection rules checked</h4>${ruleRows}<h4>Line-by-line review</h4><ol class="log-lines">${lineRows}</ol><p class="tool-privacy">This log was analyzed locally and is never transmitted or stored.</p></div>`;
  }
  if (id === 'tool-08') {
    const addressed = result.checked.length ? `<div class="device-addressed"><h4>Items addressed</h4><ul class="tool-checklist">${result.checked.map((i) => `<li class="check-pass">${icon('circle-check','w-4 h-4')}<span>${esc(i.label)}</span></li>`).join('')}</ul></div>` : '';
