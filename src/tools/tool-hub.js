...
export const TOOL_DEFINITIONS = {
  'tool-01': { name: 'Password Strength Evaluator', icon: 'key-round', kind: 'password', note: 'Use a fictional password only. Nothing entered here is stored or transmitted.' },
  'tool-02': { name: 'Phishing Email Inspector', icon: 'mail-warning', kind: 'phishing', note: 'Paste a sanitized message. Never include real credentials, codes, or personal data.' },
  'tool-03': { name: 'Safe URL & Link Validator', icon: 'link-2', kind: 'url', note: 'Use a public training URL only—not private or authenticated links.' },
  'tool-04': { name: 'Cybersecurity Quick Check', icon: 'clipboard-pen-line', kind: 'incident', note: 'Use the existing local practice check to assess and report a fictional security event.' }
  , 'tool-05': { name: 'Incident Report Assistant', icon: 'file-text', kind: 'incident-assistant', note: 'Practice structuring a report for your organization’s process. This tool never submits reports or contacts anyone.' }
  , 'tool-06': { name: 'QR Code Safety Checker', icon: 'qr-code', kind: 'qr', note: 'Decode a QR code locally, preview its destination, then check URLs with the same engine as URL Safety Checker.' }
};
...
function assistantReport(input) {
  ...
  const report = [`INCIDENT REPORT PRACTICE SUMMARY`, `Incident type: ${input.incidentType || 'Not provided'}`, `When it happened: ${input.when || 'Not provided'}`, `Affected system/device: ${input.system || 'Not provided'}`, `Actions already taken: ${input.actions || 'Not provided'}`, `Relevant details: ${input.details || 'Not provided'}`, '', 'RECOMMENDED NEXT STEPS (MODULE 7)', ...checklist.map((x) => `[ ] ${x}`), '', 'Practice only: use your organization’s designated reporting process. This tool does not submit or contact anyone.'].join('\n');
...
function resultPanel(result, id) {
  ...
  if (id === 'tool-05') return `<div class="tool-result-panel"><div class="tool-result-heading"><span class="risk-badge low">Practice summary ready</span></div><p>Your report is still only in this page session. Review it, then use your organization’s approved reporting channel yourself.</p><pre class="report-preview" data-report-text>${esc(result.report)}</pre><div class="report-actions"><button class="secondary-btn" data-copy-report>Copy report</button><button class="secondary-btn" data-download-report>Download .txt</button></div></div>`;
  return `<div class="tool-result-panel"><div class="tool-result-heading"><span class="risk-badge ${result.severity.toLowerCase()}">${esc(result.severity)} severity</span></div><p>Use the details below to practice a concise, useful incident report.</p><pre class="report-preview">${esc(result.draft)}</pre></div>`;
}

function form(id, instance) {
  if (id === 'tool-01') return `<label>Fictional password<input type="password" data-tool-input="password" autocomplete="new-password" placeholder="Training-Example-Only-2026!"></label><button class="primary-btn" data-analyze-tool="${id}">Evaluate locally ${icon('arrow-right','w-4 h-4')}</button>`;
  if (id === 'tool-02') return `<div class="tool-grid"><label>Sender email<input data-tool-input="sender" placeholder="security@example.test"></label><label>Subject<input data-tool-input="subject" placeholder="Action required"></label></div><label>Message body<textarea data-tool-input="body" placeholder="Paste a sanitized email or SMS example"></textarea></label><label>Visible links (one per line)<textarea data-tool-input="links" placeholder="https://example.test/login"></textarea></label><button class="primary-btn" data-analyze-tool="${id}">Inspect message ${icon('scan-search','w-4 h-4')}</button>`;
